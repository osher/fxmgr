        ...
        if (mustExist.length) {
            const notExist = mustExist.reduce(
                (ix, {[idAttr]: searchedId, ...searchedProps}) =>
                  ( searchedId
                    ? idMatch(docs, searchedId) || ix.byId.push(id)
                    : propsMatch(docs, searchedProps) || ix.byProps.push(props)
                  ) && ix,
                { byId: [], byProps: [] },
            )

            assert.deepEqual(notExist, { byId: [], byProps: [] }, `${dbName}.${colName}: mustExist assertion - missing mandatory entries in db`)
        }
