module.exports = ({
  ...
  batch: ({
    ...
    equal  = eql =>
      mget(
        ...
        (err, vals) => err
          ...
          : vals.forEach((v, ix) => v
              ? eql[ix].value === v
                || reject.push(`unexpected value in key: ${eql[ix].key}, expected: ${eql[ix].value}, found: ${v}`)
    ...
    exec: () =>
      new Promise((acc, rej) => multi.exec((err, res) => err ? rej(err) : acc(res)))
  ...
  useData:
    (entities, {
      ...
      ixByStageAndType =
        supportSteps.reduce(
          (ix, step) => Object.values(entities).reduce(
            (ix, {
              dataType = defaultDataType,
              [step]: data,
            }) => data && ix[step][dataType].push(data) && ix,
            ix,
          ),
      ...
      seed: keepAlive =>
        ...
        .then(connect)
        .then(() => runStep('seed'))
        .then(keepAlive ? Promise.resolve : disconnect),
